  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      if (res.page === "ALL") {
        ...
        const totalPages = Math.ceil(+countTotal / 10);
        let start = countTotal - currentPage * 10;
        if (start > 0) {
          start = countTotal - currentPage * 10;
        } else if (start < 0 && start > -10) {
          start = 0;
          perPage = countTotal - Math.floor(+countTotal / 10) * 10;
        } else {
          start = 0;
          perPage = 0;
        }
